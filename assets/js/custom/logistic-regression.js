...
(function() {
    ...
    const hypothesis = `h_{\\theta}(\\vec{x}) = g(\\vec{\\theta}\\cdot\\vec{x})`;
    katex.render(hypothesis, document.getElementById('hypothesis'));

    const thetaTx = `\\vec{\\theta}\\cdot\\vec{x} = \\theta^{T}x =
      \\theta_{0} + \\theta_{1}x_{1} + \\theta_{2}x_{2} + \\theta_{3}x_{3}`;
    katex.render(thetaTx, document.getElementById('theta-T-x'));

    const probability = `h_{\\theta}(\\vec{x}) = P(y = 1 | x;\\theta)`;
    katex.render(probability, document.getElementById('probability'));

    const costFunction = `J(\\theta) = \\left[\\frac{\\lambda}{2m}\\sum_{j = 1}^{n}
        \\theta_j^{2}\\right] - \\left[ \\frac{1}{m}\\sum_{i = 1}^{m}y^{(i)}
        log(h_{\\theta}(x^{(i)})) + (1 - y^{(i)})log(1 - h_{\\theta}(x^{(i)})) \\right]`;
    katex.render(costFunction, document.getElementById('cost-function'));

    const gradient = `\\nabla J(\\theta) = \\frac{\\partial J}
        {\\partial \\theta_{0}}\\hat{\\theta_{0}}
        + \\frac{\\partial J}{\\partial \\theta_{1}}\\hat{\\theta_{1}}
        + \\frac{\\partial J}{\\partial \\theta_{2}}\\hat{\\theta_{2}}
        + \\frac{\\partial J}{\\partial \\theta_{3}}\\hat{\\theta_{3}}`;
    katex.render(gradient, document.getElementById('gradient'));

    const gradientDescent = `\\vec{\\theta} := \\vec{\\theta} - \\alpha\\nabla J(\\theta)`;
    katex.render(gradientDescent, document.getElementById('gradient-descent'));

    const algorithm1 = `\\theta_{0} := \\theta_{0} - \\alpha \\frac{\\partial J}{\\partial
        \\theta_{0}} = \\theta_{0} - \\alpha \\left[\\frac{1}{m}\\sum_{i = 1}^{m}(h_{\\theta}
        (x^{(i)}) - y^{(i)})x_{0}^{(i)} \\right]`;
    katex.render(algorithm1, document.getElementById('algorithm-1'));

    const algorithm2 = `\\theta_{j} := \\theta_{j}
        - \\alpha \\frac{\\partial J}{\\partial \\theta_{j}}
        = \\theta_{j} - \\alpha \\left[\\frac{1}{m}\\sum_{i = 1}^{m}(h{\\theta}(x^{(i)})
            - y^{(i)})x_{j}^{(i)} + \\frac{\\lambda}{m}\\theta_{j}\\right]`;
    katex.render(algorithm2, document.getElementById('algorithm-2'));

    const params = `\\theta^{T}x = \\theta_{0} + \\theta_{1}x_{1} + \\theta_{2}x_{2} +
        \\theta_{3}x_{3} =9.294\\times 10^{-3} + 9.263\\times 10^{-3}x_{1} + 2.658x_{2}
        - 3.784\\times 10^{-1}x_{3}`;
    katex.render(params, document.getElementById('params'));

    const prediction = `P(Win) = h_{\\theta}(\\vec{x}) = \\frac{1}{1 + e^{-\\theta^{T}x}}`;
    ...
    const matchData = {
        "win": {
            "x1":[
                32.485,34.304,33.658,33.173,32.79,33.34,
            ...
            "x2":[
                3.065,3.436,3.795,3.525,1.817,2.694,2.844,3.522,1.94,2.895,3.024,2.933,
            ...
            "x3":[
                20.5,22.6,17.05,12.05,16.95,22.4,22.1,16.25,17.9,20.7,14.6,18.75,15.3,16.8,
        ...
        "loss": {
            "x1":[
                29.595,28.897,29.858,31.409,30.157,32.705,32.843,29.722,32.099,29.159,30.854,
            ...
            "x2":[
                1.953,2.16,1.483,1.829,3.426,2.898,2.68,2.36,2.84,2.386,2.13,2.306,3.241,2.714,
            ...
            "x3":[
                16.35,17.7,18.9,15.8,18,22,20.45,16.35,15.3,17.25,18.85,18.75,15.75,14.85,21.85,
